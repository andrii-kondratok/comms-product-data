...
--
--  Поріг калібрується на наших парах «стаття → пост» (posts_db/eval_coverage.py):
--  пост зроблено саме з цієї статті, тобто це рівно той випадок, який треба ловити.
-- ============================================================================

-- Вікно свіжості — доба: новина, старша за добу, для коментаря вже не годиться.
-- Перевірка за посиланням увімкнена одразу (точна), за схожістю — після калібрування.
